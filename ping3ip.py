...
class pingip(threading.Thread):
	...
	def __init__(self,bastion,ip,count=4,verbose=False,occurence=5,mode='texte',description="NONE",resultat=None):
	...
	def rebond_cisco(self,type="SSH"):
		...
		try:
			...
			if type=="TELNET":
				...
				self.proxy.sendline ('x112097')
			else:
				self.proxy.sendline ('ssh -l x112097 '+self.bastion )
			expect_value=self.proxy.expect(['assword','yes'])
			#print("COUCOU:"+expect_value.__str__())
			if expect_value==0:
				self.proxy.sendline('Tek3pmac!')
...
if __name__ == '__main__':
	...
	parser.add_argument("-r","--rebond",action="store",help="rebond/bastion/proxy",required=False,default="192.64.10.129")
