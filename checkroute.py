...
class table_routage(object):
	...
	def check_presence_from_dump(self,other,optionNoAlert=False,optionOptimisation=False,ReplaceObj=None):
		"Verifie la presence dans la table plus globale(la table reelle)"
		Message={}
		...
		for reseau__ in self.tab_prefix:
			...
			#print(reseau)
			if reseau in other:
				#other_reseau=other.tab_prefix_dict[other_reseau__.str_short()]
			...
			#if reseau.nexthop[0]=='192.64.128.111':
			#	
			...
			if not optionOptimisation and not test_route_exacte:	
				...
				if test_route_plus_globale:
					...
					if best_route.nexthop==reseau.nexthop:
...
class table_routage_allVRF(object):
	...
	def getAllVRF(self):
		return self.dict_RT_AllVRF.keys()
...
if __name__ == '__main__':
	...
	parser.add_option('-o','--optimisation',help='Mode optimisé ne check que la route précise',dest='OPTIMISATION',action='store',default=False)
	...
	if not (opts.CIBLE_VRF=="ALL" or opts.COURANT_VRF=="ALL") :
		...
		if opts.REPLACE_FILE:
			...
			CIBLE.affichage_check_rt(CIBLE.check_presence_from_dump(COURANT,optionNoAlert=opts.NO_ALERT,optionOptimisation=opts.OPTIMISATION))
	else:
