...
import random
import queue
from ParsingShow import ParseVrf , ParseBgpNeighbor
...
class equipement_connus(object):
	...
	def append_read(self,nom_equipement_):
		...
		while not ( OS_=="IOS" or OS_=="Nexus" or OS_=="XR" or OS_=="OLD-IOS" or OS_=="ACE"):
			OS_=input("Quel est l'OS [XR|IOS|Nexus|OLD-IOS|ACE](Nexus):")
			if not OS_:
				OS_="Nexus"
...
class connexion(threading.Thread):
	...
	def __init__(self,equipement,queue__,queue_echec,liste_commande,mode="SSH",output="NONE",repertoire="",commande_en_ligne="",retry=0,status="NOT INITIATED",verbose=False,timeout=300):
		...
		self.mode=mode
		self.rebond=pxssh.pxssh(timeout=timeout,maxread=100000,options={"ServerAliveInterval":5},searchwindowsize=10)
		if queue__==None:
	...
	def launch_commandes(self):
		try:
			...
			self.rebond.login('192.64.10.129','x112097','B2rn1s12345!')
			if self.verbose:
				self.rebond.logfile = sys.stdout.buffer
	...
	def launch_commande_en_ligne(self):
		...
		try:
			...
			self.rebond.login(self.equipement.IP,'x112097','Tek3pmac!')
