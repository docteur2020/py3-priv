...
if __name__ == '__main__':
	...
	if args.vlan:
		if not args.xclude:
			result=dc.extract_port_vlans(args.vlan)
		else:
			result=exclude_cdpneigbor(dc.extract_port_vlans(args.vlan),DC_cdp(dump=args.xclude),dc)
	elif args.mac:
		if not args.xclude:
		...
		with open(args.liste_mac_vlan) as fichier:
			...
			for info__ in list_vlan_mac:
				...
				mac=info__[1]
				if not args.xclude:
					result_cur=dc.extract_macs(mac,vlan=vlan)
					for entry in result_cur:
						if not re.search('ODIN-C',entry[3],re.IGNORECASE):
							if args.cmp_mac:
					...
					result_cur=exclude_cdpneigbor(dc.extract_macs(mac,vlan=vlan),DC_cdp(dump=args.xclude),dc)
					info_cur=[]
					print(result_cur)
					for entry in result_cur:			
						if not re.search('ODIN-C',entry[3],re.IGNORECASE):
