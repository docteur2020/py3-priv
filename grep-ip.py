...
def search(network,lines,option_v=False,option_e=False,option_n=False,option_l=False,option_p=False):
	result=io.StringIO()
	mask_possible=genere_mask_list()
	wildcard_possible=genere_wildcard_list()
	number=0
				...
				if possible_mask in mask_possible:
					...
					if option_e:
						if net_cur_line == network and number not in number_line_list_printed:
							if option_n:
								if option_p:
									result.write (str(number)+"-EXACT MATCH("+str(network)+"):"+line)
								else:
									print (str(number)+"-EXACT MATCH:"+line)
							else:
								if option_p:
									result.write ("EXACT MATCH("+str(network)+"):"+line)
								else:
									result.write ("EXACT MATCH:"+line)
							if number not in number_line_list_printed:
									number_line_list_printed.append(number)
						...
						if network in net_cur_line and network >= net_cur_line and number not in number_line_list_printed:
							...
							if option_n:
								if option_p:
									result.write(str(number)+"-MATCH("+str(network)+"):"+line)
								else:
									result.write(str(number)+"-MATCH:"+line)
							else:
								if option_p:
									result.write("MATCH("+str(network)+"):"+line)
								else:
									result.write("MATCH:"+line)
							if number not in number_line_list_printed:
									number_line_list_printed.append(number)
					...
					if option_l:
						if match_wildcard_longer(network,res_cur_line,wild_cur_line,option_v,option_e) and number not in number_line_list_printed:
							if option_n:
								if option_p:
									result.write(str(number)+"-MATCH("+str(network)+"):(WILDCARD):"+line)
								else:
									result.write(str(number)+"-MATCH (WILDCARD):"+line)
							else:
								if option_p:
									result.write("MATCH("+str(network)+"):(WILDCARD):"+line)
								else:
									result.write("MATCH (WILDCARD):"+line)
							if number not in number_line_list_printed:
								number_line_list_printed.append(number)
					else:					
						if match_wildcard(network,res_cur_line,wild_cur_line,option_v,option_e) and number not in number_line_list_printed:
					...
					if net_cur_line == network and number not in number_line_list_printed:
						if option_n:
							if option_p:
								...
								result.write (str(number)+"-EXACT MATCH:"+line)
						else:
							if option_p:
								result.write ("EXACT MATCH("+str(network)+"):"+line)
							else:
...
if __name__ == '__main__':
	...
	parser.add_argument("-r", "--recursive",action="store_true",default=False, help="directory",required=False)
	...
	if mode=="FILE":
		...
		if args.recursive:
			all_lines={}
			listFile=glob.glob(args.file+'/**/*.*',recursive=True)
			for filename in listFile:
				with open(filename,'r') as file:
					try:
						all_lines[filename]=file.read()	
					except UnicodeDecodeError as E:
						print(f'Unicode Error for {filename} :{E}')
			...
			with open(args.network[0]) as file_network:
				...
				for net__ in liste_network:
					try:
						...
						listNetwork.append(param_network)
					except:
						raise grepIpError(code=2,value1=args.network)
		else:
			listNetwork=[ Network(net__) for net__ in  args.network ]
			

		for filename in all_lines:
			for net__ in listNetwork:
				matchStr=search(net__,all_lines[filename],args.Verbose,args.exact,args.numeric,args.longer_prefixes,option_p=args.printing)
				if matchStr:
					print(filename+':')
					result[filename]=matchStr
					print(matchStr)
