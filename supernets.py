...
def process_input_file(file__): 
    """ Read each network from file and compare to the current supernet.""" 
	
    pdb.set_trace()
	
    with open(file__,'r') as file__r:
        lines=file__r.read().splitlines()
...
def main__(file_input,file_output):

	global networks 
	global verbose_output

	process_input_file(file_input)
	process_prefixes()
	
	resultat=[]
	
	for network in sorted(networks, key=lambda ip: ip.network_address.packed): 
		resultat.append(str(network))
		
	with open(file_output,'w') as file__w:
		file__w.writelines([ str(net__)+"\n" for net__ in  resultat ] )
